...
export type FluidOrbProps = ComponentProps<"div"> & {
  size?: number;
  color?: string;
  /** Advance the shader clock. When false, the orb holds a still frame. */
  animated?: boolean;
};
...
/**
 * Animated WebGL fluid orb. Self-contained: owns its canvas, GL program
 * and clock loop. `animated` only starts/stops the clock — it never
 * recompiles shaders, so play/pause toggles stay cheap.
 */
export function FluidOrb({
  ...
  // GL setup — reruns only when canvas size or color change.
  useEffect(() => {
    ...
    glRef.current = { gl, program, vert, frag, buffer, uTime };
    // Frozen first frame so the orb is never blank before the loop starts.
  ...
  // Clock loop — toggling `animated` never touches GL resources.
  // Accumulated time (not wall-clock) so pause/resume never jumps.
  useEffect(() => {
    const state = glRef.current;
    if (!state) return;
    if (!animated) {
      state.gl.uniform1f(state.uTime, timeRef.current);
